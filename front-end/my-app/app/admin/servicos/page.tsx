...
export default function AdminServicos() {
  ...
  async function carregarDetalhe(id: number) {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`https://protoserv-backend.up.railway.app/servicos/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error();
  ...
  async function salvarServico() {
    ...
    try {
      ...
      const url = isEditar
        ? `https://protoserv-backend.up.railway.app/servicos/${servicoSelecionado!.id}`
        : "https://protoserv-backend.up.railway.app/servicos";
  ...
  async function toggleAtivo(s: Servico) {
    try {
      ...
      const res = await fetch(`https://protoserv-backend.up.railway.app/servicos/${s.id}/${acao}`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error();
